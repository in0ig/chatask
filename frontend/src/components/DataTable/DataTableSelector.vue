<template>
  <el-dialog
    v-model="visible"
    :title="'数据表选择（已选：' + selectedCount + '张表）'"
    width="600px"
    :before-close="handleClose"
    :destroy-on-close="true"
    class="data-table-selector-dialog"
      ...
      <div class="quick-filters">
        ...
        >
          <el-option
            ...
            :value="source.value"
          >
            <el-tag :type="getDataSourceTypeColor(source.type)" size="small">
              {{ source.label }}
            </el-tag>
          </el-option>
        </el-select>
            ...
            <el-dropdown @command="handleSort">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item command="dataSource">按数据源</el-dropdown-item>
                  <el-dropdown-item command="fieldCount">按字段数</el-dropdown-item>
                ...
                <div class="table-info">
                  <div class="table-header">
                    ...
                    <div class="table-badges">
                      <el-tag 
                        :type="getDataSourceTypeColor(table.dataSourceType)" 
                        size="small"
                        class="type-tag"
                      >
      ...
      <div class="footer-actions">
        <div class="selection-summary">
          <el-tag v-if="selectedCount > 0" type="success" size="large">
            ...
            已选择 {{ selectedCount }} 张数据表
          </el-tag>
          <span v-else class="no-selection">请选择数据表</span>
        </div>
...
import { 
  ...
  Close, View, Plus, ArrowDown, Grid, MoreFilled, Check 
...
const sortBy = ref<'name' | 'dataSource' | 'fieldCount' | 'selected'>('name')
const highlightedId = ref<string | null>(null)
const isConfirming = ref(false)
const loadingProgress = ref(0)

// 计算属性：总数统计
const totalCount = computed(() => props.dataTables.length)
const pinnedCount = computed(() => props.dataTables.filter(t => t.pinned).length)

// 计算属性：可用的数据源
const availableDataSources = computed(() => {
  const sources = [...new Set(props.dataTables.map(t => ({
...
const filteredTables = computed(() => {
  ...
  if (selectedDataSources.value.length > 0) {
    filtered = filtered.filter(table => selectedDataSources.value.includes(table.dataSourceId))
  }
  ...
  if (searchKeyword.value) {
    ...
    filtered = filtered.filter(table => {
      return table.name.toLowerCase().includes(keyword) ||
             ...
             table.dataSourceName.toLowerCase().includes(keyword) ||
  ...
  return [...filteredTables.value].sort((a, b) => {
    ...
    switch (sortBy.value) {
      case 'selected':
        ...
        break
      case 'dataSource':
        const sourceCompare = a.dataSourceName.localeCompare(b.dataSourceName)
...
const selectedCount = computed(() => {
  return props.dataTables.filter(table => table.selected).length
})
...
// 处理表格悬停
const handleTableHover = (table: DataTable) => {
...
// 处理单个表的选择变化
const handleTableSelectionChange = (table: DataTable, value: boolean) => {
...
const selectAll = () => {
  ...
  ElMessage.success(`已选择 ${filteredTables.value.length} 张数据表`)
}

const selectNone = () => {
  props.dataTables.forEach(table => {
...
const pinSelected = () => {
  const selectedTables = props.dataTables.filter(t => t.selected)
  selectedTables.forEach(table => {
    table.pinned = true
  })
  ElMessage.success(`已置顶 ${selectedTables.length} 张数据表`)
}

// 切换置顶状态
const togglePin = (table: DataTable) => {
  table.pinned = !table.pinned
  ElMessage.success(table.pinned ? `${table.name} 已置顶` : `${table.name} 已取消置顶`)
}

// 处理表格操作
const handleTableAction = (table: DataTable, command: string) => {
  switch (command) {
    case 'info':
      emit('info', table)
      break
...
const getEmptyStateSubtitle = () => {
  ...
  if (filterType.value === 'selected') {
    return '请先选择一些数据表，然后可以在这里查看'
  }
  if (filterType.value === 'pinned') {
    return '您可以通过点击星标图标来置顶常用的数据表'
  }
  return '系统中还没有配置任何数据表，请先添加数据表'
}
...
// 预览单个表
const previewTable = (table: DataTable) => {
  emit('preview', table)
}
...
const confirmSelection = async () => {
  ...
  try {
    ...
    const selectedTables = props.dataTables.filter(table => table.selected)
    emit('confirm', selectedTables)
    
    ElMessage.success(`已确认选择 ${selectedTables.length} 张数据表`)
